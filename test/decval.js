...
describe('Validation tests', function(){
	...
	it('not a number should not be ok', function(done){
		...
		ok.should.be.false();
		ok = decval.validate('0.12a');
		ok.should.be.false();
		ok = decval.validate('0.1a');
		ok.should.be.false();
		done();
	});

	it('less decimals than maximum should be ok', function(done){
		var ok = decval.validate('123.4');
		ok.should.be.true();
		var ok = decval.validate('123.4', 3, 3);
		ok.should.be.true();
		var ok = decval.validate('123.45', 3, 3);
		ok.should.be.true();
		done();
	});
